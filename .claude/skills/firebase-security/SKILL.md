...
- `setAdminAccess` grants/revokes the `admin` custom claim server-side immediately, but the client only picks it up on its next ID token refresh (`src/store/authSlice.js`'s `readAdminClaim` calls `getIdTokenResult(user)` without forcing a refresh) — roughly hourly, or on next sign-in. This is expected, not a bug: `isAdmin` only gates client-side UI (`RequireAdmin` in `src/navigation/AppRoutes.jsx`), every admin callable independently re-checks the claim server-side via `assertAdmin`, so a stale client-side claim can only delay the admin UI appearing, never grant unauthorized access. Don't "fix" this by forcing a token refresh unless a real UX complaint asks for it.
