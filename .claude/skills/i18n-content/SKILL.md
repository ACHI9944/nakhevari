---
description: Use when adding or changing Georgian/English copy, translation keys in src/translations/resources.js, or any user-facing label/validation/error text. Covers keeping both languages in sync and status-label wording.
---
...
- Keep status labels aligned with project values: listing `pending`, `published`, `rejected`, `draft`; company `pending`, `verified`, `rejected`, `not_required`; user profile `active`, `suspended`.
